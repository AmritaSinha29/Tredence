...
const STATS = [
  { label: 'Active Workflows', value: '12', icon: Activity, color: '#7c6cf0', bg: '#f5f3fe' },
  { label: 'Templates Available', value: '5', icon: Layers, color: '#4a8ff7', bg: '#f0f5ff' },
  { label: 'Completed Today', value: '8', icon: CheckCircle, color: '#22a86b', bg: '#e7f8f0' },
  { label: 'Avg. Completion', value: '2.4h', icon: Clock, color: '#e89e1c', bg: '#fef8eb' },
];

const RECENT = [
  { name: 'Employee Onboarding — Q2', nodes: 6, status: 'active', updated: '2 hours ago' },
  { name: 'Annual Leave Request Flow', nodes: 7, status: 'draft', updated: '5 hours ago' },
  { name: 'Document Verification', nodes: 5, status: 'active', updated: '1 day ago' },
  { name: 'Exit Process Workflow', nodes: 8, status: 'draft', updated: '3 days ago' },
];
    ...
    <div className="max-w-[1200px] mx-auto">
      ...
      <div className="mb-8">
        ...
        <p className="text-sm text-[#8e90a6] mt-1">
          Design, validate, and simulate HR workflows — all in one place.
        </p>
      </div>
      ...
      <div className="grid grid-cols-4 gap-4 mb-8">
        {STATS.map((s) => (
          <div key={s.label} className="bg-white rounded-xl border border-[#e2e4ef] p-4 flex items-center gap-4 shadow-sm">
          ...
          <div className="space-y-2">
            ...
            <Link to="/templates"
              ...
              <div>
                ...
                <div className="text-[11px] text-[#8e90a6]">Pre-built workflows</div>
              </div>
            </Link>
            <Link to="/designer"
              className="flex items-center gap-3 p-3.5 bg-white border border-[#e2e4ef] rounded-xl shadow-sm hover:bg-[#f8f9fc] transition-colors">
        ...
        <div className="col-span-2">
          ...
          <div className="bg-white rounded-xl border border-[#e2e4ef] shadow-sm overflow-hidden">
            <table className="w-full">
              ...
              <tbody>
                {RECENT.map((r, i) => (
                  <tr key={i} className="border-b border-[#e2e4ef] last:border-0 hover:bg-[#f8f9fc] transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <Link to="/designer" className="text-xs text-[#7c6cf0] hover:underline font-medium">Open</Link>
                    </td>
                  </tr>
                ))}
              </tbody>
